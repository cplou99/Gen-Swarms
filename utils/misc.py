...
class CheckpointManager(object):
    ...
    def prune(self, n_keep):
        # Sort the checkpoints based on the validation metric in descending order
        self.ckpts.sort(key=lambda ckpt: ckpt['score'], reverse=False)

        # Keep only the top n checkpoints
        keep_ckpts = self.ckpts[:n_keep]

        # Delete the remaining checkpoints from the disk and the ckpts list
        for ckpt in self.ckpts[n_keep:]:
            os.remove(os.path.join(self.save_dir, ckpt['file']))

        # Update the ckpts list
        self.ckpts = keep_ckpts

    def save(self, model, args, score, others=None, step=None, n_keep=5):
