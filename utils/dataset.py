...
def farthest_point_sampling(point_cloud, num_samples):
    ...
    # Initialize with the first point in the point cloud
    sampled_points[0] = point_cloud[np.random.randint(N)]
    distances = np.linalg.norm(point_cloud - sampled_points[0], axis=1)

    for i in range(1, num_samples):
        # Select the farthest point
        # print("Maximum distance for sampled point", i, "is", np.max(distances))
        farthest_point_index = np.argmax(distances)
        sampled_points[i] = point_cloud[farthest_point_index]
...
class ShapeNetCore(Dataset):
    ...
    def load(self):
        ...
        with h5py.File(self.path, mode='r') as f:
            print(f"Loading {self.split} set from {self.path} and reshaping point clouds following", self.scale_mode, "strategy...")
            for pc, pc_id, cate_name in _enumerate_pointclouds(f):
                ...
                # print("Pc before reshape min and max", pc.min(), pc.max())
                # print("Shift and scale", shift, scale)
                pc = (pc - shift) / scale
                # print("Pc after reshape min and max", pc.min(), pc.max())
                if self.num_points != 2048:
                    print("Sampling", self.num_points, "points from", pc.shape)
                    pc = farthest_point_sampling(pc.numpy(), self.num_points)
                    pc = torch.from_numpy(pc).float()

                vel = torch.zeros_like(pc)
